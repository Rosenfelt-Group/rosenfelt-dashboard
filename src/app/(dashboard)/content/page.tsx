        ...
        <div key={idea.id} className="card flex items-start gap-4">
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            {idea.status === "in_progress" && (
              <span className={clsx("badge", STATUS_STYLES.in_progress)}>In progress</span>
            )}
            {idea.status === "published" && (
              <span className={clsx("badge", STATUS_STYLES.published)}>Published</span>
            )}
          </div>
        </div>
      ))}
...
function BlogsTab({ ideas }: { ideas: ContentIdea[] }) {
  ...
  const blogs = ideas.filter(i => i.signal_type === "blog_topic" || i.status === "published" || i.status === "in_progress");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1">
        {(["all", "in_progress", "published"] as const).map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            ...
            {s === "all" ? ` (${blogs.length})` : s === "published" ? ` (${published})` : ` (${inProgress})`}
          </button>
        ))}
      </div>
            ...
            <div key={blog.id}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-2 mt-1.5 flex-wrap">
                  ...
                  {blog.post_id && (
                    <span className="text-xs text-brand-muted">Post #{blog.post_id}</span>
                  )}
                </div>
              </div>
  ...
  return (
    <div className="p-4 md:p-8 pb-24 md:pb-8 max-w-4xl space-y-5">
      ...
      <div className="flex gap-1 border-b border-brand-border">
        {([
          { key: "ideas",   label: `Ideas (${ideas.filter(i => i.status === "queued").length})` },
          { key: "blogs",   label: `Blogs (${ideas.filter(i => i.signal_type === "blog_topic" || ["in_progress","published"].includes(i.status)).length})` },
      ...
      {tab === "ideas"   && <ContentIdeasTab ideas={ideas} onUpdate={updateStatus} />}
      {tab === "blogs"   && <BlogsTab ideas={ideas} />}
      {tab === "website" && <WebsitePagesTab />}
    </div>
  );
}
