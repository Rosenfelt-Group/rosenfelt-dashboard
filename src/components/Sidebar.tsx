    ...
    <aside className="hidden md:flex flex-col w-[220px] min-w-[220px] bg-brand-black h-screen fixed top-0 left-0 z-20">
                ...
                return (
                  <Link
                    ...
                    className={clsx(
                      ...
                      active
                        ? "bg-brand-orange/18 text-brand-orange-light"
                        : "text-white/48 hover:text-white/80 hover:bg-white/6"
                    )}
                  >
                    <span className={clsx("shrink-0", active ? "text-brand-orange" : "opacity-50")}>
      ...
      <div className="px-3 py-3 border-t border-white/8 flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[12px] font-semibold text-white/85 truncate">Brian Rosenfelt</div>
          <div className="text-[10px] text-white/32">Owner</div>
        </div>
        <NotificationBell />
        <button
          onClick={handleLogout}
          title="Sign out"
          className="p-1 rounded text-white/35 hover:text-white/70 hover:bg-white/10 transition-colors"
        >
