...
const mobileNav = ["", "tasks", "approvals", "content", "agents"];
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-52 bg-white border-r border-brand-border flex-col z-10">
        <div className="px-5 py-5 border-b border-brand-border">{brand}</div>
        ...
        <div className="px-3 py-3 border-t border-brand-border">
          <button onClick={() => signOut({ callbackUrl: "/login" })}
            className="flex items-center gap-2.5 w-full px-3 py-2 rounded-lg text-sm text-brand-muted hover:bg-brand-offwhite hover:text-brand-black transition-colors">
            <Icon name="logout" />
            Sign out
          </button>
        </div>
      ...
      {/* ── Mobile top bar ── */}
      <header className="md:hidden fixed top-0 inset-x-0 h-12 bg-white border-b border-brand-border flex items-center px-4 z-20">
        {brand}
      </header>

      {/* ── Mobile bottom tab bar ── */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-brand-border flex items-center justify-around z-20 pb-safe">
        {nav.filter(n => mobileNav.includes(n.href.replace(/^\//, ""))).map(item => (
          <Link key={item.href} href={item.href}
            className={clsx(
              "flex flex-col items-center gap-0.5 px-3 py-2 min-w-[56px] transition-colors",
              isActive(item.href) ? "text-brand-orange" : "text-brand-muted"
            )}>
            <Icon name={item.icon} />
